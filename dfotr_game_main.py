...
class AlgoGame:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("The Pathfinder! by Zahracolt")

        # Initialize clock
        self.clock = pygame.time.Clock()
        self.total_time = 0
        self.red = (255, 140, 100)
        self.green = (0, 255, 0)
    ...
    def _update_screen(self):
        ...
        if not self.scrolling:
            image = self.settings.font.render(f'{"You fell off the path! " if self.game_started else ""}'
                                              f'Press any arrow key to {"begin" if not self.game_started else "continue"}', True, (255, 255, 254))
            self.screen.blit(image, (self.screen.get_width() * 0.05, self.screen.get_height() * 0.08))

            # Show before game start only
            if not self.game_started:
                image = self.settings.fancy_font.render('"The Pathfinder!"', True, (255, 255, 255))
                self.screen.blit(image, (self.screen.get_width() * 0.05, self.screen.get_height() * 0.75))
                image = self.settings.fancy_font.render('   By: Zahracolt', True, (255, 250, 250))
    ...
    def _draw_text(self):
        ...
        image = self.settings.font.render(f'Your Score: {score}', True, text_color)
        self.screen.blit(image, (self.screen.get_width() * 0.8, 0))

        # Lives Counter
        image = self.settings.font.render(f'Your Lives Remaining: {num_lives}', True, text_color)
